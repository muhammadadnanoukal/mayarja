...
class PaymentTransaction(models.Model):
    ...
    def action_demo_set_done(self):
        ...
        """
        self.ensure_one()
        if self.provider_code != 'syriatell12':
            return

        notification_data = {'reference': self.reference, 'simulated_state': 'done'}
        self._handle_notification_data('syriatell12', notification_data)
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError("syriatell12: " + _("The transaction is not linked to a token."))

        simulated_state = self.token_id.syriatell12_simulated_state
    ...
    def _send_refund_request(self, **kwargs):
        ...
        refund_tx._handle_notification_data('syriatell12', notification_data)
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'syriatell12' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'syriatell12')])
        if not tx:
            raise ValidationError(
                "syriatell12: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_notification_data(self, notification_data):
        ...
        self.provider_reference = f'syriatell12-{self.reference}'
        ...
        if state == 'pending':
            ...
            self._set_error(_("You selected the following syriatell12 payment status: %s", state))
