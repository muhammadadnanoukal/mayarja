...
class SyriatelCashApi(object):
    ...
    def get_token(self, username, password):
        ...
        response = None
        try:
            response = requests.post(self.get_token_url, json=get_token_params, headers=headers, verify=False)
            ...
            raise requests.exceptions.ConnectionError("Timeout Error :", errt)
        except requests.exceptions.RequestException as err:
            raise requests.exceptions.ConnectionErrorprint("OOps: Something Else, ", err)
    ...
    def payment_request(self, customerMSISDN, merchantMSISDN, amount, transaction_id, token):
        ...
        print('payment_request_params==>',payment_request_params)
        response = None
        try:
            response = requests.post(self.payment_request_url, json=payment_request_params, headers=headers, verify=False)
            if response.status_code == 200:
                response_dict = json.loads(response.text)
    ...
    def payment_confirmation(self, OTP, merchantMSISDN, transactionID, token):
        ...
        response = None
        try:
            response = requests.post(self.payment_confirmation_url, json=payment_confirmation_params, headers=headers, verify=False)
            if response.status_code == 200:
                response_dict = json.loads(response.text)
                if response_dict.get('errorCode') == '0':
                    return True
                    ...
                    raise Exception("Customer MSISDN does not have customer wallet !!")
                elif response_dict.get('errorCode') == '-13':
                    raise Exception("Customer MSISDN does not have enough balance !!")
    ...
    def resend_OTP(self, merchantMSISDN, transactionID, token):
        ...
        response = None
        try:
            response = requests.post(self.resend_OTP_url, json=resend_OTP_params, headers=headers, verify=False)
            if response.status_code == 200:
                response_dict = json.loads(response.text)
                if response_dict.get('errorCode') == '0':
                    print('HERE')
